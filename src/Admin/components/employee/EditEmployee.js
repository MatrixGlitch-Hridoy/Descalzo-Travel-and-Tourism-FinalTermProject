...
export default function EditEmployee() {
  ...
  useEffect(() => {
    ...
    .then(respnse =>{
     ...
     const e_id=window.location.pathname.split("/employee/update/")[1];
    // console.log(props);
    // console.log(props.match);
    //  console.log(window.location.pathname.split("/employee/update/")[1]);
    // console.log(props);
    axios.get(`http://127.0.0.1:8000/api/employee/${e_id}`)
    .then(respnse =>{
        if(respnse.data.success===true){
            setEmployee(respnse.data.users);
...
const updateEmployee= (e) =>{
  ...
  axios.post(`http://127.0.0.1:8000/api/employee/update/${e_id}`, formData)
  .then(res => 
  {
...
if (loading) {
    return <h1>Loading Edit  Employee.......</h1>
  }
  return (
    ...
    <h4>Update Employee
    <Link to="/employee" className="btn btn-primary btn-sm float-end"> View Enployee</Link>
    </h4>
  
    </div>
    <div className="card-body">
      <form onSubmit={updateEmployee} id="E_FORM">
         <div>
    ...
    <div className="form-group mb-3">
        <div>
              <label htmlfor="name"> Name</label>
               <input className="form-control"onChange={handleInput} value={employeeInput.name} name="name" readOnly/>
    ...
    <div className="tab-pane card-body border fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
      <div className="form-group mb-3">
          <div>
                <label htmlfor="username">Username</label>
                 <input className="form-control" onChange={handleInput} value={employeeInput.username} name="username" readOnly/>
      ...
      <div className="form-group mb-3">
       <div>
             ...
             <input type="hidden" className="form-control" onChange={handleInput} value={employeeInput.password} name="password"/>
      ...
      <div className="form-group mb-3">
       <div>
             ...
             <input className="form-control" onChange={handleInput} value={employeeInput.account} name="account" readOnly/>
       </div>
      </div>
      <div className="form-group mb-3">
      <div>
    ...
    <div className="form-group mb-3">
          <div>
                <label htmlfor="phone">Phone</label>
                 <input className="form-control" onChange={handleInput} value={employeeInput.phone} name="phone" readOnly/>
          </div>
      </div>
      <div className="form-group mb-3">
       <div>
             <label htmlfor="email">Email</label>
             <input className="form-control" onChange={handleInput} value={employeeInput.email} name="email" readOnly/>
       </div>
      </div>
      <div className="form-group mb-3">
       <div>
             <label htmlfor="address">Address</label>
             <input className="form-control" onChange={handleInput} value={employeeInput.address} name="address" readOnly/>
       </div>
      </div>
          ...
          <button type="submit" className="btn btn-primary px-4 mt-2">UpdateEmployee</button>
       </form>
    </div>
